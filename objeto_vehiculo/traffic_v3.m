...
mainvars = zeros(9, n_obj); %xpos, ypos, xvel, yvel, xacc, yacc, xobs?, yobs?
id = 1;
vehicle_length = 20;
...
mainvars(:,id) = [intersection(1); intersection(2); 0; 0; 0; 0; 0; 1; 0];
id = id + 1;
for n = 1:n_horz_veh
    h_objects(:,id) = vehicle_init(0, intersection(2), vehicle_length, 'right', rand(1,3));
    mainvars(:,id) = [0; intersection(2); velocity; 0; acceleration; 0; 1; 0; 1];
    id = id + 1;
end
for n = 1:n_vert_veh
    h_objects(:,id) = vehicle_init(intersection(1), 0, vehicle_length, 'down', rand(1,3));
    mainvars(:,id) = [intersection(1); 0; 0; velocity; 0; acceleration; 0; 1; 2];
    id = id + 1;
end
...
s = 1;
init_time = randi([1,100],[1,n_horz_veh+n_vert_veh])*time_step;
for t = 0:time_step:duration
    for car = 2:(n_horz_veh+n_vert_veh+1)
        if init_time(car - 1) < t
            [delta, v] = vehicle_dynamics(car, [0; 0], mainvars(3:4,car), mainvars(5:6,car), time_step, mainvars, vehicle_length);
